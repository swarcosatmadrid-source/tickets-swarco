...
try:
    ...
    cargar_estilos()
    # Conexión a Google Sheets (definida en tus Secrets)
...
# Esta función viene de tu archivo usuarios.py
if gestionar_acceso(conn):
    
    # Si el usuario se loguea con éxito, el programa continúa aquí:
    ...
    with col_add:
        if st.button("➕ Añadir otro equipo a este ticket", use_container_width=True):
            if len(ns_in) >= 3 and len(falla_in) >= 10:
                ...
                st.error("⚠️ Complete el N.S. y la descripción antes de añadir.")
        ...
        if st.button("🚀 GENERAR TICKET FINAL", type="primary", use_container_width=True):
            ...
            if data_final and proyecto_ub:
                ...
                if enviar_email_outlook(d_cli['Empresa'], d_cli['Contacto'], proyecto_ub, data_final, d_cli['Email'], ticket_id, tel_final):
                    ...
                    # 2. GUARDADO EN GOOGLE SHEETS (Lectura/Escritura mediante el conector)
                    try:
                        ...
                        # Leemos la pestaña de tickets (Sheet1) y actualizamos
                        df_h = conn.read(worksheet="Sheet1", ttl=0)
                        df_updated = pd.concat([df_h, nueva_fila], ignore_index=True)
                        conn.update(worksheet="Sheet1", data=df_updated)
                        
                        st.success(f"Ticket {ticket_id} enviado y registrado correctamente.")
                        st.balloons()
                        st.session_state.lista_equipos = []
                    except Exception as e:
                        st.warning(f"Ticket enviado por email, pero hubo un error al registrarlo en el Excel: {e}")
            else:
                st.error("⚠️ Falta la ubicación o los datos del equipo para procesar el envío.")
